import graph_search_improved


def run_path_coverage(map_path, cliff_height):
    g = graph_search_improved.GridMap(map_path, cliff_height)
    g.display_cell_values()
    # g.display_cell_height()

    # res = graph_search_improved.path_coverage(g.init_pos, g.transition, graph_search_improved._ACTIONS)
    # if res[2] is None:
    #     g.display_map(res[0], res[1])
    #     print(res[0], res[1])
    # else:
    #     g.display_map(res[0][1], res[1])
    #     print(res[0][1], res[1])


if __name__ == '__main__':
    run_path_coverage('./map0.txt', 1.0)

